...
const cardReader = require('../lib/cardReader');
...
cardReader.selectDevice().then(([device]) => {
	cardReader.open(device).then(() => {
		console.log(`Device ${device}  open.`);

		cardReader.getCardType()
			.then(types => {
				switch(types.type) {
					...
					case CardType.ContactIC:
						...
						cardReader.SIMPowerOn(SIMNumber.ContactCPUCard)
							.then(ATR => {
								console.log(`SIM ATR: ${ATR}`);
								console.log('Send SIM APDU: 00A404000E315041592E5359532E444446303100');
								cardReader.SIMSendAPDU(SIMNumber.ContactCPUCard, '00A404000E315041592E5359532E444446303100')
									.then(recvData => {
										console.log(`SIM data: ${recvData}`);
										cardReader.SIMPowerOff(SIMNumber.ContactCPUCard).then(() => {
				...
				switch(types.magnetic) {
					...
					case MagneticCardType.HasReadableMagneticCardInfo:
						...
						cardReader.readMagneticCard(cardReader.MagneticCard.Track1)
							.then(data => {
						...
						cardReader.readMagneticCard(cardReader.MagneticCard.Track2)
							.then(data => {
						...
						cardReader.readMagneticCard(cardReader.MagneticCard.Track3)
							.then(data => {
						...
						break;
					default:
						console.log('Unknown magnetic band card status');
						break;
				}
			})
			.catch(err => {
